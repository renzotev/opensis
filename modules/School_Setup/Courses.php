...
//			$header .= '<TD>' . SelectInput($RET['GRADE_SCALE_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][GRADE_SCALE_ID]','Grading Scale',$options,'Not Graded') . '</TD>';
			...
			foreach($options_RET as $option)
				$options[$option['CALENDAR_ID']] = $option['TITLE'];
                        ...
                        $header .= '<TD>' . SelectInput($RET['CALENDAR_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][CALENDAR_ID]','Calendar',$options,false,'id=def_cal onClick=show_this_msg("def_cal","cal_msg","calendar");') . '</TD>';
                        else
			...
			$header .= '<TD valign=top>' . CheckboxInput($RET['DOES_ATTENDANCE'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][DOES_ATTENDANCE]','Takes Attendance',$checked,$new,'<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>','<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>',true,'id="cp_does_attendance" onclick="formcheck_periods_attendance_F2(this);"') . '<br><div id="ajax_output"></div></TD>';
                        $header .= '<TD valign=top>' . CheckboxInput($RET['DOES_HONOR_ROLL'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][DOES_HONOR_ROLL]','Affects Honor Roll',$checked,$new,'<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>','<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>') . '</TD>';
			$header .= '<TD valign=top>' . CheckboxInput($RET['HALF_DAY'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][HALF_DAY]','Half Day',$checked,$new,'<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>','<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>') . '</TD>';
